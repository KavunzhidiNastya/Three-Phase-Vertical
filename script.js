...
calculateBtn.addEventListener("click", () => {
    ...
    const s2 = Math.pow(((pg/(pl-pg))*(cd6/dml)), 0.5)
    ...
    var dt = 84
    const hohw84 = 6690 * (tro*qo)+(trw*qw)/(0.12*Math.pow*(dt, 2)) 
    document.getElementById("hohw84").innerText = hohw84.toFixed(2);
    var dt = 90
    const hohw90 = 6690 * (tro*qo)+(trw*qw)/(0.12*Math.pow*(dt, 2)) 
    document.getElementById("hohw90").innerText = hohw90.toFixed(2);
    var dt = 96
    const hohw96 = 6690 * (tro*qo)+(trw*qw)/(0.12*Math.pow*(dt, 2)) 
    document.getElementById("hohw96").innerText = hohw96.toFixed(2);
    var dt = 102
    const hohw102 = 6690 * (tro*qo)+(trw*qw)/(0.12*Math.pow*(dt, 2)) 
  ...
  //  resultDiv.textContent = `${filteredNumbers.join(", ")}`;
    ...
    // Display the result
    resultDisplay.textContent = sr84, sr90, sr96, sr102;
});
